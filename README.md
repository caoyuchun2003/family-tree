...
- `PUT /people/{id}`：更新人物资料和父级关系，自动回到待审核状态
- `POST /review`：审核人物，`decision` 使用 `confirm` 或 `reject`

后续可扩展 `/relationships`、`/materials` 和 `/upload-url`。服务器上的数据库和上传文件不应直接暴露给浏览器。
...
- 人物详情侧栏
- 成员搜索和成员管理列表
- 新增和编辑成员（可选择上一级成员；无 API 时保存到浏览器 localStorage）
- 审核中心支持确认归档和退回补充
- 资料库入口（资料上传仍待接入）
- GitHub Pages 静态发布配置
